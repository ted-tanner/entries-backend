use zeroize::Zeroize;

// This file is @generated by prost-build.
#[derive(Zeroize, Clone, Copy, PartialEq, ::prost::Message)]
pub struct Timestamp {
    #[prost(uint64, required, tag = "1")]
    pub secs: u64,
    #[prost(uint32, required, tag = "2")]
    pub nanos: u32,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct Uuid {
    #[prost(bytes = "vec", required, tag = "1")]
    pub value: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct Category {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(message, required, tag = "2")]
    pub container_id: Uuid,
    #[prost(bytes = "vec", required, tag = "3")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "4")]
    pub version_nonce: i64,
    #[prost(message, required, tag = "5")]
    pub modified_timestamp: Timestamp,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct Entry {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(message, required, tag = "2")]
    pub container_id: Uuid,
    #[prost(message, optional, tag = "3")]
    pub category_id: ::core::option::Option<Uuid>,
    #[prost(bytes = "vec", required, tag = "4")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "5")]
    pub version_nonce: i64,
    #[prost(message, required, tag = "6")]
    pub modified_timestamp: Timestamp,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct AuthStringAndEncryptedPasswordUpdate {
    #[prost(string, required, tag = "1")]
    pub user_email: ::prost::alloc::string::String,
    #[prost(string, required, tag = "2")]
    pub otp: ::prost::alloc::string::String,
    #[prost(bytes = "vec", required, tag = "3")]
    pub new_auth_string: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "4")]
    pub auth_string_hash_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "5")]
    pub auth_string_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "6")]
    pub auth_string_hash_threads: i32,
    #[prost(int32, required, tag = "7")]
    pub auth_string_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "8")]
    pub password_encryption_key_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "9")]
    pub password_encryption_key_mem_cost_kib: i32,
    #[prost(int32, required, tag = "10")]
    pub password_encryption_key_threads: i32,
    #[prost(int32, required, tag = "11")]
    pub password_encryption_key_iterations: i32,
    #[prost(bytes = "vec", required, tag = "12")]
    pub encrypted_encryption_key: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerAccessTokenList {
    #[prost(string, repeated, tag = "1")]
    pub tokens: ::prost::alloc::vec::Vec<::prost::alloc::string::String>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct CategoryId {
    #[prost(message, required, tag = "1")]
    pub value: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct CategoryUpdate {
    #[prost(message, required, tag = "1")]
    pub category_id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "3")]
    pub version_nonce: i64,
    #[prost(int64, required, tag = "4")]
    pub expected_previous_version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct CategoryWithTempId {
    #[prost(int32, required, tag = "1")]
    pub temp_id: i32,
    #[prost(bytes = "vec", required, tag = "2")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "3")]
    pub version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct CredentialPair {
    #[prost(string, required, tag = "1")]
    pub email: ::prost::alloc::string::String,
    #[prost(bytes = "vec", required, tag = "2")]
    pub auth_string: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "3")]
    pub nonce: i32,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EncryptedBlobAndCategoryId {
    #[prost(bytes = "vec", required, tag = "1")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "2")]
    pub version_nonce: i64,
    #[prost(message, optional, tag = "3")]
    pub category_id: ::core::option::Option<Uuid>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EncryptedBlobUpdate {
    #[prost(bytes = "vec", required, tag = "1")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "2")]
    pub version_nonce: i64,
    #[prost(int64, required, tag = "3")]
    pub expected_previous_version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EmailChangeRequest {
    #[prost(string, required, tag = "1")]
    pub new_email: ::prost::alloc::string::String,
    #[prost(bytes = "vec", required, tag = "2")]
    pub auth_string: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EntryAndCategory {
    #[prost(bytes = "vec", required, tag = "1")]
    pub entry_encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "2")]
    pub entry_version_nonce: i64,
    #[prost(bytes = "vec", required, tag = "3")]
    pub category_encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "4")]
    pub category_version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EntryId {
    #[prost(message, required, tag = "1")]
    pub value: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EntryUpdate {
    #[prost(message, required, tag = "1")]
    pub entry_id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "3")]
    pub version_nonce: i64,
    #[prost(int64, required, tag = "4")]
    pub expected_previous_version_nonce: i64,
    #[prost(message, optional, tag = "5")]
    pub category_id: ::core::option::Option<Uuid>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct NewContainer {
    #[prost(bytes = "vec", required, tag = "1")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "2")]
    pub version_nonce: i64,
    #[prost(message, repeated, tag = "3")]
    pub categories: ::prost::alloc::vec::Vec<CategoryWithTempId>,
    #[prost(bytes = "vec", required, tag = "4")]
    pub user_public_container_key: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct NewEncryptedBlob {
    #[prost(bytes = "vec", required, tag = "1")]
    pub value: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "2")]
    pub version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct NewUser {
    #[prost(string, required, tag = "1")]
    pub email: ::prost::alloc::string::String,
    #[prost(bytes = "vec", required, tag = "2")]
    pub auth_string: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "3")]
    pub auth_string_hash_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "4")]
    pub auth_string_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "5")]
    pub auth_string_hash_threads: i32,
    #[prost(int32, required, tag = "6")]
    pub auth_string_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "7")]
    pub password_encryption_key_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "8")]
    pub password_encryption_key_mem_cost_kib: i32,
    #[prost(int32, required, tag = "9")]
    pub password_encryption_key_threads: i32,
    #[prost(int32, required, tag = "10")]
    pub password_encryption_key_iterations: i32,
    #[prost(bytes = "vec", required, tag = "11")]
    pub recovery_key_hash_salt_for_encryption: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "12")]
    pub recovery_key_hash_salt_for_recovery_auth: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "13")]
    pub recovery_key_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "14")]
    pub recovery_key_hash_threads: i32,
    #[prost(int32, required, tag = "15")]
    pub recovery_key_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "16")]
    pub recovery_key_auth_hash: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "17")]
    pub encryption_key_encrypted_with_password: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "18")]
    pub encryption_key_encrypted_with_recovery_key: ::prost::alloc::vec::Vec<u8>,
    #[prost(message, required, tag = "19")]
    pub public_key_id: Uuid,
    #[prost(bytes = "vec", required, tag = "20")]
    pub public_key: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "21")]
    pub preferences_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "22")]
    pub preferences_version_nonce: i64,
    #[prost(bytes = "vec", required, tag = "23")]
    pub user_keystore_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "24")]
    pub user_keystore_version_nonce: i64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct NewUserPublicKey {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub value: ::prost::alloc::vec::Vec<u8>,
    #[prost(message, required, tag = "3")]
    pub expected_previous_public_key_id: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct Otp {
    #[prost(string, required, tag = "1")]
    pub value: ::prost::alloc::string::String,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct PublicKey {
    #[prost(bytes = "vec", required, tag = "1")]
    pub value: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct RecoveryKeyAuthAndPasswordUpdate {
    #[prost(bytes = "vec", required, tag = "1")]
    pub recovery_key_hash_for_recovery_auth: ::prost::alloc::vec::Vec<u8>,
    #[prost(string, required, tag = "2")]
    pub user_email: ::prost::alloc::string::String,
    /// Optionally allow user to update their email address
    #[prost(string, optional, tag = "3")]
    pub new_user_email: ::core::option::Option<::prost::alloc::string::String>,
    #[prost(bytes = "vec", required, tag = "4")]
    pub new_auth_string: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "5")]
    pub new_auth_string_hash_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "6")]
    pub new_auth_string_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "7")]
    pub new_auth_string_hash_threads: i32,
    #[prost(int32, required, tag = "8")]
    pub new_auth_string_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "9")]
    pub new_recovery_key_hash_salt_for_encryption: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "10")]
    pub new_recovery_key_hash_salt_for_recovery_auth: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "11")]
    pub new_recovery_key_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "12")]
    pub new_recovery_key_hash_threads: i32,
    #[prost(int32, required, tag = "13")]
    pub new_recovery_key_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "14")]
    pub new_recovery_key_auth_hash: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "15")]
    pub encryption_key_encrypted_with_new_password: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "16")]
    pub encryption_key_encrypted_with_new_recovery_key: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct RecoveryKeyUpdate {
    #[prost(string, required, tag = "1")]
    pub otp: ::prost::alloc::string::String,
    #[prost(bytes = "vec", required, tag = "2")]
    pub recovery_key_hash_salt_for_encryption: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "3")]
    pub recovery_key_hash_salt_for_recovery_auth: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "4")]
    pub recovery_key_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "5")]
    pub recovery_key_hash_threads: i32,
    #[prost(int32, required, tag = "6")]
    pub recovery_key_hash_iterations: i32,
    #[prost(bytes = "vec", required, tag = "7")]
    pub recovery_key_auth_hash: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "8")]
    pub encrypted_encryption_key: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct UserInvitationToContainer {
    #[prost(string, required, tag = "1")]
    pub recipient_user_email: ::prost::alloc::string::String,
    #[prost(message, required, tag = "2")]
    pub recipient_public_key_id_used_by_sender: Uuid,
    #[prost(message, required, tag = "3")]
    pub recipient_public_key_id_used_by_server: Uuid,
    #[prost(bytes = "vec", required, tag = "4")]
    pub sender_public_key: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "5")]
    pub encryption_key_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "6")]
    pub container_info_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "7")]
    pub sender_info_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "8")]
    pub share_info_symmetric_key_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(message, required, tag = "9")]
    pub expiration: Timestamp,
    #[prost(bool, required, tag = "10")]
    pub read_only: bool,
}
#[derive(Zeroize, Clone, Copy, PartialEq, ::prost::Message)]
pub struct AcceptKeyInfo {
    #[prost(bool, required, tag = "1")]
    pub read_only: bool,
    #[prost(uint64, required, tag = "2")]
    pub expiration: u64,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct Container {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub encrypted_blob: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "3")]
    pub version_nonce: i64,
    #[prost(message, required, tag = "4")]
    pub modified_timestamp: Timestamp,
    #[prost(message, repeated, tag = "5")]
    pub categories: ::prost::alloc::vec::Vec<Category>,
    #[prost(message, repeated, tag = "6")]
    pub entries: ::prost::alloc::vec::Vec<Entry>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerFrame {
    #[prost(message, required, tag = "1")]
    pub access_key_id: Uuid,
    #[prost(message, required, tag = "2")]
    pub id: Uuid,
    #[prost(message, repeated, tag = "3")]
    pub category_ids: ::prost::alloc::vec::Vec<ContainerFrameCategory>,
    #[prost(message, required, tag = "4")]
    pub modified_timestamp: Timestamp,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerFrameCategory {
    #[prost(int32, required, tag = "1")]
    pub temp_id: i32,
    #[prost(message, required, tag = "2")]
    pub real_id: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerIdAndEncryptionKey {
    #[prost(message, required, tag = "1")]
    pub container_id: Uuid,
    #[prost(message, required, tag = "2")]
    pub container_access_key_id: Uuid,
    #[prost(bytes = "vec", required, tag = "3")]
    pub encryption_key_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bool, required, tag = "4")]
    pub read_only: bool,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerList {
    #[prost(message, repeated, tag = "1")]
    pub containers: ::prost::alloc::vec::Vec<Container>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerShareInvite {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub container_accept_key_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "3")]
    pub container_accept_key_id_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "4")]
    pub container_info_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "5")]
    pub sender_info_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "6")]
    pub container_accept_key_info_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(bytes = "vec", required, tag = "7")]
    pub share_info_symmetric_key_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(message, required, tag = "8")]
    pub recipient_public_key_id_used_by_sender: Uuid,
    #[prost(message, required, tag = "9")]
    pub recipient_public_key_id_used_by_server: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ContainerShareInviteList {
    #[prost(message, repeated, tag = "1")]
    pub invites: ::prost::alloc::vec::Vec<ContainerShareInvite>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct EntryIdAndCategoryId {
    #[prost(message, required, tag = "1")]
    pub entry_id: Uuid,
    #[prost(message, required, tag = "2")]
    pub category_id: Uuid,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct InvitationId {
    #[prost(message, required, tag = "1")]
    pub value: Uuid,
}
#[derive(Zeroize, Clone, Copy, PartialEq, ::prost::Message)]
pub struct IsUserListedForDeletion {
    #[prost(bool, required, tag = "1")]
    pub value: bool,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct ServerErrorResponse {
    #[prost(
        enumeration = "ErrorType",
        required,
        tag = "1",
        default = "ActixWebPrehandler"
    )]
    pub err_type: i32,
    #[prost(string, required, tag = "2")]
    pub err_message: ::prost::alloc::string::String,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct SigninNonceAndHashParams {
    #[prost(bytes = "vec", required, tag = "1")]
    pub auth_string_hash_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "2")]
    pub auth_string_hash_mem_cost_kib: i32,
    #[prost(int32, required, tag = "3")]
    pub auth_string_hash_threads: i32,
    #[prost(int32, required, tag = "4")]
    pub auth_string_hash_iterations: i32,
    #[prost(int32, required, tag = "5")]
    pub nonce: i32,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct SigninToken {
    #[prost(string, required, tag = "1")]
    pub value: ::prost::alloc::string::String,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct TokenPair {
    #[prost(string, required, tag = "1")]
    pub access_token: ::prost::alloc::string::String,
    #[prost(string, required, tag = "2")]
    pub refresh_token: ::prost::alloc::string::String,
    #[prost(message, required, tag = "3")]
    pub server_time: Timestamp,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct AuthenticatedSession {
    #[prost(message, required, tag = "1")]
    pub tokens: TokenPair,
    #[prost(bytes = "vec", required, tag = "2")]
    pub preferences_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "3")]
    pub preferences_version_nonce: i64,
    #[prost(bytes = "vec", required, tag = "4")]
    pub user_keystore_encrypted: ::prost::alloc::vec::Vec<u8>,
    #[prost(int64, required, tag = "5")]
    pub user_keystore_version_nonce: i64,
    #[prost(bytes = "vec", required, tag = "6")]
    pub password_encryption_key_salt: ::prost::alloc::vec::Vec<u8>,
    #[prost(int32, required, tag = "7")]
    pub password_encryption_key_mem_cost_kib: i32,
    #[prost(int32, required, tag = "8")]
    pub password_encryption_key_threads: i32,
    #[prost(int32, required, tag = "9")]
    pub password_encryption_key_iterations: i32,
    #[prost(bytes = "vec", required, tag = "10")]
    pub encryption_key_encrypted_with_password: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, PartialEq, ::prost::Message)]
pub struct UserPublicKey {
    #[prost(message, required, tag = "1")]
    pub id: Uuid,
    #[prost(bytes = "vec", required, tag = "2")]
    pub value: ::prost::alloc::vec::Vec<u8>,
}
#[derive(Zeroize, Clone, Copy, PartialEq, ::prost::Message)]
pub struct VerificationEmailSent {
    #[prost(bool, required, tag = "1")]
    pub email_sent: bool,
    #[prost(uint64, required, tag = "2")]
    pub email_token_lifetime_hours: u64,
}
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, PartialOrd, Ord, ::prost::Enumeration)]
#[repr(i32)]
pub enum ErrorType {
    /// 400
    IncorrectlyFormed = 0,
    InvalidMessage = 1,
    OutOfDate = 2,
    InvalidState = 3,
    MissingHeader = 4,
    ConflictWithExisting = 5,
    /// 401
    IncorrectCredential = 6,
    TokenExpired = 7,
    TokenMissing = 8,
    WrongTokenType = 9,
    /// 403
    UserDisallowed = 10,
    PendingAction = 11,
    IncorrectNonce = 12,
    TooManyAttempts = 13,
    ReadOnlyAccess = 14,
    /// 404
    UserDoesNotExist = 15,
    KeyDoesNotExist = 16,
    ContainerDoesNotExist = 17,
    EntryDoesNotExist = 18,
    CategoryDoesNotExist = 19,
    InvitationDoesNotExist = 20,
    ForeignKeyDoesNotExist = 21,
    /// 413
    InputTooLarge = 22,
    /// 418
    TooManyRequested = 23,
    /// 500
    InternalError = 24,
    /// Anything that Actix Web returns before the handler is reached
    ActixWebPrehandler = 25,
}
impl ErrorType {
    /// String value of the enum field names used in the ProtoBuf definition.
    ///
    /// The values are not transformed in any way and thus are considered stable
    /// (if the ProtoBuf definition does not change) and safe for programmatic use.
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Self::IncorrectlyFormed => "INCORRECTLY_FORMED",
            Self::InvalidMessage => "INVALID_MESSAGE",
            Self::OutOfDate => "OUT_OF_DATE",
            Self::InvalidState => "INVALID_STATE",
            Self::MissingHeader => "MISSING_HEADER",
            Self::ConflictWithExisting => "CONFLICT_WITH_EXISTING",
            Self::IncorrectCredential => "INCORRECT_CREDENTIAL",
            Self::TokenExpired => "TOKEN_EXPIRED",
            Self::TokenMissing => "TOKEN_MISSING",
            Self::WrongTokenType => "WRONG_TOKEN_TYPE",
            Self::UserDisallowed => "USER_DISALLOWED",
            Self::PendingAction => "PENDING_ACTION",
            Self::IncorrectNonce => "INCORRECT_NONCE",
            Self::TooManyAttempts => "TOO_MANY_ATTEMPTS",
            Self::ReadOnlyAccess => "READ_ONLY_ACCESS",
            Self::UserDoesNotExist => "USER_DOES_NOT_EXIST",
            Self::KeyDoesNotExist => "KEY_DOES_NOT_EXIST",
            Self::ContainerDoesNotExist => "CONTAINER_DOES_NOT_EXIST",
            Self::EntryDoesNotExist => "ENTRY_DOES_NOT_EXIST",
            Self::CategoryDoesNotExist => "CATEGORY_DOES_NOT_EXIST",
            Self::InvitationDoesNotExist => "INVITATION_DOES_NOT_EXIST",
            Self::ForeignKeyDoesNotExist => "FOREIGN_KEY_DOES_NOT_EXIST",
            Self::InputTooLarge => "INPUT_TOO_LARGE",
            Self::TooManyRequested => "TOO_MANY_REQUESTED",
            Self::InternalError => "INTERNAL_ERROR",
            Self::ActixWebPrehandler => "ACTIX_WEB_PREHANDLER",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
        match value {
            "INCORRECTLY_FORMED" => Some(Self::IncorrectlyFormed),
            "INVALID_MESSAGE" => Some(Self::InvalidMessage),
            "OUT_OF_DATE" => Some(Self::OutOfDate),
            "INVALID_STATE" => Some(Self::InvalidState),
            "MISSING_HEADER" => Some(Self::MissingHeader),
            "CONFLICT_WITH_EXISTING" => Some(Self::ConflictWithExisting),
            "INCORRECT_CREDENTIAL" => Some(Self::IncorrectCredential),
            "TOKEN_EXPIRED" => Some(Self::TokenExpired),
            "TOKEN_MISSING" => Some(Self::TokenMissing),
            "WRONG_TOKEN_TYPE" => Some(Self::WrongTokenType),
            "USER_DISALLOWED" => Some(Self::UserDisallowed),
            "PENDING_ACTION" => Some(Self::PendingAction),
            "INCORRECT_NONCE" => Some(Self::IncorrectNonce),
            "TOO_MANY_ATTEMPTS" => Some(Self::TooManyAttempts),
            "READ_ONLY_ACCESS" => Some(Self::ReadOnlyAccess),
            "USER_DOES_NOT_EXIST" => Some(Self::UserDoesNotExist),
            "KEY_DOES_NOT_EXIST" => Some(Self::KeyDoesNotExist),
            "CONTAINER_DOES_NOT_EXIST" => Some(Self::ContainerDoesNotExist),
            "ENTRY_DOES_NOT_EXIST" => Some(Self::EntryDoesNotExist),
            "CATEGORY_DOES_NOT_EXIST" => Some(Self::CategoryDoesNotExist),
            "INVITATION_DOES_NOT_EXIST" => Some(Self::InvitationDoesNotExist),
            "FOREIGN_KEY_DOES_NOT_EXIST" => Some(Self::ForeignKeyDoesNotExist),
            "INPUT_TOO_LARGE" => Some(Self::InputTooLarge),
            "TOO_MANY_REQUESTED" => Some(Self::TooManyRequested),
            "INTERNAL_ERROR" => Some(Self::InternalError),
            "ACTIX_WEB_PREHANDLER" => Some(Self::ActixWebPrehandler),
            _ => None,
        }
    }
}
