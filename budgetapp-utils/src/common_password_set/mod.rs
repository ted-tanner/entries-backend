...
impl CommonPasswordSet {
    pub fn generate(common_passwords_file_path: PathBuf) -> CommonPasswordSet {
        let file_error_msg = format!("Failed to open {}", common_passwords_file_path.display());
        let common_passwords_file = File::open(common_passwords_file_path).expect(&file_error_msg);
