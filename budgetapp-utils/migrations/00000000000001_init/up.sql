...
CREATE TABLE budget_share_invites (
    ...
    -- The corresponding public key is in the budget_share_keys table.
    --
    ...
    budget_share_private_key_encrypted BYTEA NOT NULL,
    ...
    -- Information about the budget_share_private_key, such as its expiration, whether it is
    -- read-only, and the key_id
    budget_share_private_key_info_encrypted BYTEA NOT NULL,
...
-- These public keys allow the server to verify that the user with the private key has
-- permission to join a budget.
CREATE TABLE budget_share_keys (
    key_id UUID UNIQUE NOT NULL,
    budget_id UUID NOT NULL,
...
CREATE INDEX ON budget_share_keys USING HASH (key_id);
...
ALTER TABLE budget_share_keys ADD CONSTRAINT budget_key FOREIGN KEY(budget_id) REFERENCES budgets(id) ON DELETE CASCADE;
